    ...
    def match_spectrum_peptide(self, psm) -> Dict:
        ...
        try:
            ...
            match_info = {
                ...
                "modified_peptide": getattr(
                    psm.peptide, "mod_peptide", getattr(psm.peptide, "peptide", None)
                ),
                "charge": getattr(psm, "charge", None),
                "scan_num": getattr(psm, "scan_num", None),
                "spectrum_native_id": getattr(
                    psm, "spectrum_native_id", getattr(psm.spectrum, "native_id", None)
                ),
...
def parallel_process(
    data: List[Any], worker_class: Any, num_threads: Optional[int] = None
) -> List[Any]:
    """
...
def parallel_spectrum_matching(
    psms: List, config: Dict, num_threads: Optional[int] = None
) -> List[Dict]:
    """
